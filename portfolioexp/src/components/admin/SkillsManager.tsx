...
const CATEGORIES: SkillCategory[] = [
  "FRONTEND","BACKEND","DEVOPS","DATABASE","MOBILE","DESIGN","TESTING","OTHER",
];

const CATEGORY_COLORS: Record<string, string> = {
  FRONTEND: "text-blue-400",  BACKEND: "text-green-400",
  DEVOPS:   "text-rose-400",  DATABASE: "text-amber-400",
  MOBILE:   "text-purple-400",DESIGN: "text-pink-400",
  TESTING:  "text-teal-400",  OTHER: "text-slate-400",
};

const empty = { name: "", category: "FRONTEND" as SkillCategory, level: 80, yearsOfExperience: 1 };
      ...
      <div className="card p-5 space-y-4">
        ...
        <div className="grid sm:grid-cols-4 gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-xs text-accent w-8 text-right">{form.level}%</span>
          </div>
          <button
            onClick={addSkill}
      ...
      <div className="grid md:grid-cols-2 gap-5">
        {CATEGORIES.filter((cat) => grouped[cat]?.length > 0).map((cat) => (
          <div key={cat} className="card p-4 space-y-3">
            <h3 className={`text-xs font-mono font-semibold uppercase tracking-wider ${CATEGORY_COLORS[cat]}`}>
              {cat}
            </h3>
